...
class Role extends Admin
{
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!isset($data['mobile_menu_auth'])) {
                $data['mobile_menu_auth'] = [];
            } else {
                $data['mobile_menu_auth'] = explode(',', $data['mobile_menu_auth']);
            }
            // 验证
        ...
        //手机菜单列表
        $mobile_menus = cache('access_mobile_menus');

        if (!$mobile_menus) {
            $modules = implode(array_flip(config('mobie_menu_list')),',');
            $mobile_menus = MobilemenuModel::where('module', 'in', $modules)->order('sort,id')->column('id,pid,sort,url_value,title,icon');
           
            $mobile_menus = Tree::toLayer($mobile_menus);
            $mobile_menus = $this->buildJsTree($mobile_menus);

            // 非开发模式，缓存菜单
            if (config('develop_mode') == 0) {
                cache('access_mobile_menus',$mobile_menus);
            }
        }
        ...
        $this->assign('mobile_menus', $mobile_menus);
        return $this->fetch();
    }
    ...
    public function edit($id = null)
    {
        ...
        $mobile_modules = implode(array_flip(config('mobie_menu_list')),',');
        $mobile_menus = MobilemenuModel::where('module', 'in', $mobile_modules)->order('sort,id')->column('id,pid,sort,url_value,title,icon');
        $mobile_menus = Tree::toLayer($mobile_menus);
        $mobile_menus = $this->buildJsTree($mobile_menus,$info,1);
     ...
     * @param number $type 0pc节点 1手机节点
     * @author 蔡伟明 <314013107@qq.com>
     * @return string
     */
    private function buildJsTree($menus = [], $user = [],$type=0)
    {
        $result = '';
        if (!empty($menus)) {
            $option = [
            ...
            foreach ($menus as $menu) {
                ...
                if (isset($user['menu_auth']) && $type==0 && !empty($user['menu_auth'])) {
                    $option['selected'] = in_array($menu['id'], $user['menu_auth']) ? true : false;
                }
                if (isset($user['mobile_menu_auth']) && $type==1 && !empty($user['mobile_menu_auth'])) {
                    $option['selected'] = in_array($menu['id'], $user['mobile_menu_auth']) ? true : false;
                }
                if (isset($menu['child'])) {
                    $result .= '<li id="'.$menu['id'].'" data-jstree=\''.json_encode($option).'\'>'.$menu['title'].($menu['url_value'] == '' ? '' : ' ('.$menu['url_value'].')').$this->buildJsTree($menu['child'], $user,$type).'</li>';
                } else {
