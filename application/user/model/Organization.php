...
class Organization extends Model
{
     ...
     * @return mixed
     */
    public static function getMenuTree($id = 0, $default = '',$flag = 0)
    {
        if($flag) $result[0]       = '顶级节点';
        $where['status'] = ['egt', 0];
        ...
        // 获取节点
       // $menus = self::where($where)->order('pid,id')->column('id,pid,title');var_dump($menus);exit;
        
        $menus = Tree::toList(self::where($where)->order('pid,id')->column('id,pid,title,title as name'));
    ...
    public static function getMenuTree2($id = 0, $default = '',$flag = 0)
    {
    	if($flag) $result[0]       = '顶级节点';
    	$where['status'] = ['egt', 0];
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        if (!$menus) {
            ...
            $menus = self::where($map)->order('sort,id')->column('id,pid,title');
     ...
     * @return array
     */
    public static function getMenusByGroup($fields = true, $map = [])
    {      
    ...
    public static function getGroup()
    {
        $map['status'] = 1;
        $map['pid']    = 0;
        $menus = self::where($map)->order('id,sort')->column('title');
        return $menus;
    }
