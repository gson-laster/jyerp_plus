...
class User extends Model
{
	...
	public static function getUser(){
		$map['status'] = 1;
		$data_list = self::where($map)->column('id,nickname');
		return $data_list;
	}
	
	  public static function getList($map = [])
    {
        $data_list = self::view('admin_user', true)
        ->view("admin_organization", ['title'], 'admin_organization.id=admin_user.organization', 'left')
        ->view('admin_attachment',['id'=>"zid"],'admin_user.avatar=admin_attachment.id','left')
        ->view('admin_position',['title'=>'ztitle'],'admin_user.position=admin_position.id')   
        ->where($map)
        ->paginate();
        return $data_list;
    }
