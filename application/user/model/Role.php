...
class Role extends Model
{
    ...
    // 写入时，将菜单id转成json格式
    public function setMobileMenuAuthAttr($value)
    {
        return json_encode($value);
    }

    // 读取时，将菜单id转为数组
    public function getMobileMenuAuthAttr($value)
    {
        return json_decode($value, true);
    }
     ...
     * @return mixed
     */
    public static function getTree2()
    {
     ...
     * @return mixed
     */
    public function mobileRoleAuth()
    {
        $menu_auth = cache('role_menu_auth_'.session('user_auth.role'));
        if (!$menu_auth) {
            $menu_auth = self::where('id', session('user_auth.role'))->value('mobile_menu_auth');
            $menu_auth = json_decode($menu_auth, true);
            $menu_auth = MenuModel::where('id', 'in', $menu_auth)->column('id,url_value');
        }
        // 非开发模式，缓存数据
