 ...
 */
class Mobilemenu extends Admin
{
    /**
     * 手机节点首页
    ...
    public function index($group = 1)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($modules) {
                ...
                $MobilemenuModel = new MobilemenuModel();
                if (false !== $MobilemenuModel->saveAll($data)) {
                    $this->success('保存成功');
                } else {
                    $this->error('保存失败');
                }
        ...
        // 配置分组信息
        $list_group = MobilemenuModel::getGroup();
        ...
        if ($group == 'module-sort') {
            $map['status'] = 1;
            $map['pid']    = 0;
            $modules = MobilemenuModel::where($map)->order('sort,id')->column('icon,title', 'id');
            $this->assign('modules', $modules);
        } else {
            // 获取节点数据
            $data_list = MobilemenuModel::getMenusByGroup($group);
     ...
     * @return mixed
     */
    public function add($module = 'message', $pid = '')
    {
        // 保存数据
        if ($this->request->isPost()) {
            ...
            if ($menu = MobilemenuModel::create($data)) {
                // 自动创建子节点
        ...
        return ZBuilder::make('form')
            ->setPageTitle('新增节点')
            ->addLinkage('module', '所属模块', '', config('mobie_menu_list'), $module, url('ajax/getModuleMobileMenus'), 'pid')
            ->addFormItems([
                ['select', 'pid', '所属节点', '所属上级节点', MobilemenuModel::getMenuTree(0, '', $module), $pid],
                ['text', 'title', '节点标题'],
            ...
            ->addColorpicker('color', '请选择颜色', '', 'blue')
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!MobilemenuModel::where($map)->find()) {
                MobilemenuModel::changeModule($data['id'], $data['module']);
            }

            if (MobilemenuModel::update($data)) {
                Cache::clear();
                // 记录行为
                // $details = '节点ID('.$id.')';
        ...
        return ZBuilder::make('form')
            ...
            ->addLinkage('module', '所属模块', '',config('mobie_menu_list'), '', url('ajax/getModuleMobileMenus'), 'pid')
            ->addFormItem('select', 'pid', '所属节点', '所属上级节点', MobilemenuModel::getMenuTree(0, '', $info['module']))
    ...
    private function setRoleMenu($role_id = '', $roles = [])
    {
        ...
        // 该节点的所有子节点，包括本身节点
        $menu_child   = MobilemenuModel::getChildsId($role_id);
        $menu_child[] = (int)$role_id;
        // 该节点的所有上下级节点
        $menu_all = MobilemenuModel::getLinkIds($role_id);
    ...
    public function delete($record = [])
    {
        ...
        $menu = MobilemenuModel::where('id', $id)->find();

        if ($menu['system_menu'] == '1') $this->error('系统节点，禁止删除');

        // 获取该节点的所有后辈节点id
        $menu_childs = MobilemenuModel::getChildsId($id);

        // 要删除的所有节点id
        $all_ids = array_merge([(int)$id], $menu_childs);

        // 删除节点
        if (MobilemenuModel::destroy($all_ids)) {
            Cache::clear();
            // 记录行为
    ...
    private function createChildNode($data = [], $pid = '')
    {
        ...
        if ($child_node) {
            $MobilemenuModel = new MobilemenuModel();
            $MobilemenuModel->insertAll($child_node);
        }
    }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        if (false !== MobilemenuModel::where('id', $id)->setField('status', $status)) {
            Cache::clear();
            // 记录行为日志
            if (!empty($record)) {
