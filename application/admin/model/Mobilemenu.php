...
{
    // 设置当前模型对应的完整数据表名称
    protected $table = '__ADMIN_MOBILEMENU__';
    ...
    public static function getTopMenu($max = '', $cache_tag = '')
    {
        ...
        if (!$menus) {
            ...
            $map['status'] = 1;
            $map['pid']    = 0;
            $menus = self::where($map)->order('sort,id')->limit($max)->column('id,pid,module,title,url_value,url_type,url_target,icon,params,color');
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        if (!$menus) {
            ...
            $menus = self::where($map)->order('sort,id')->column('id,pid,module,title,url_value,url_type,url_target,icon,params,color');
     ...
     * @return array
     */
    public static function getLinkId($map)
    {
