...
class Materials extends Admin
{
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
			...
			$data['code'] = 'BOM'.date('YmdHid',time()); 
            // 验证
...
EOF;
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (MaterialsModel::update($data)) {
            	
            	$mlist = explode(',',$data['materials_list']);
            	$oldmlist = explode(',',$data['old_materials_list']);
            	$dif = array_diff($oldmlist,$mlist);
            	MaterialsListModel::where(['smid'=>['in',$dif],'pmid'=>$id])->delete();
            	...
            	foreach($data['mid'] as $k => $v){
            		...
            		if($data['mlid'][$k]){
            			...
            			MaterialsListModel::update($info);
            		}else{         			
            			$info = [
            					'pmid'=>$data['id'],
            					'smid'=>$v,
                ...
                // 记录行为
            	$details    = '详情：清单ID('.$data['id'].'),修改人ID('.UID.')';
                action_log('produce_waterials_edit', 'produce_waterials', $id, UID, $details);
    ...
    $("#old_materials_list").val($("#materials_list").val());    			     		 		
     $.ajax({
			type: "GET",
			async: false,
			url: "/admin.php/produce/materials/tech/pmid/"+pmid+"/materials_list/"+materials_list,
...
EOF;
        ...
        $data_list = MaterialsModel::getOne($id);
        	...
        	->addGroup(
        			[
        				'编辑物料清单' =>[
        							...
        							['hidden', 'pid'],
			            			['text', 'code','BOM编号'],
			            			['select', 'type','BOM类型','',['0'=>'工程BOM','1'=>'生产BOM','2'=>'销售BOM','3'=>'成本BOM']],
			            			['text', 'name','主题'],
			            			['text','version', '版本'],
			            			['text','technology_line_name', '工艺路线'],
        							['text','pid_name', '父件','请选择父件'],
     ...
     * @return mixed
     */
    public function tech($pmid = '',$materials_list = '')
    {
    	if($materials_list == '') {
    		$html = '';	
    	}else{
    		$map = ['produce_materials_list.pmid'=>$pmid,'stock_material.id'=>['in',($materials_list)]];
    		$order = '';
    		$data = $data_list = MaterialsListModel::getList($map,$order);
    		$html = '<div class="form-group col-md-12 col-xs-12" id="form_group_materials_name"><table class="table table-bordered"><tbody><tr><td>子件名称</td><td>规格</td><td>单位</td><td>定额</td><td>损耗率</td><td>是否关键件</td><td>使用状态</td><td>备注</td><td>操作</td></tr>';
    		foreach ($data as $k => $v){ 
    			$html.='<tr><input type="hidden" name="mid[]" value="'.$v['smid'].'"><input type="hidden" name="mlid[]" value="'.$v['id'].'"><td>'.$v['name'].'</td><td>'.$v['version'].'</td><td>'.$v['unit'].'</td><td><input type="text" name="quota[]" value="'.$v['quota'].'"></td><td><input type="number" name="loss_rate[]" value="'.$v['loss_rate'].'"></td><td><select name="is_key[]">';
    			if($v['is_key'] == 1){
    				$html.='<option value="1" selected>是</option><option value="0">否</option>';		
    			}else{
    				$html.='<option value="1">是</option><option value="0" selected>否</option>';
    			}
    			$html .='</select></td><td><select name="mstatus[]">';
    			if($v['status'] == 1){
    				$html.='<option value="1" selected>启用</option><option value="0">停用</option>';
    			}else{
    				$html.='<option value="1">启用</option><option value="0" selected>停用</option>';
    			}
    			$html .='</select></td><td><input type="text" name="mnote[]" value="'.$v['note'].'"></td><td><a href="javascript:;" onclick="delMaterials(this,\''.$v['smid'].'\')">删除</a></td></tr>';
    		}   		
	...
	$('#pick').click(function(){
    		...
    		if($("#form_group_materials_name",parent.document).length>0){
	    		...
	    		chk.each(function(){
	    			...
	    			html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><input type="hidden" name="mlid[]" value="0"><td>'+$.trim($(this).find('td').eq(2).text())+'</td><td>'+$.trim($(this).find('td').eq(6).text())+'</td><td>'+$.trim($(this).find('td').eq(5).text())+'</td><td><input type="text" name="quota[]"></td><td><input type="number" name="loss_rate[]"></td><td><select name="is_key[]"><option value="1">是</option><option value="0">否</option></select></td><td><select name="mstatus[]"><option value="1">启用</option><option value="0">停用</option></select></td><td><input type="text" name="mnote[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
