...
use app\contract\model\Hire as HireModel;
use app\contract\model\Hiredetail;
class Hire extends Admin
{
    public function index()
    {
        ...
        $map = $this->getMap();
        // 排序
        $order = $this->getOrder('contract_hire.id desc');
        ...
        return ZBuilder::make('table')
            ->setSearch(['contract_hire.name' => '合同名称','supplier_list.name'=>'供应商']) // 设置搜索框
    ...
    public function add(){
        ...
        if($this->request->isPost()){
            ...
            if($model = HireModel::create($data)){
							 flow_detail($data['name'],'contract_hire','contract_hire','contract/hire/task_list',$model['id']);
                //记入行为
...
EOF;
    ...
    public function tech($pid = '',$materials_list = '')
    {
        if($materials_list == '' || $materials_list == 'undefined') {
            ...
            $map = ['contract_hire_detail.pid'=>$pid,'stock_material.id'=>['in',($materials_list)]];
            ...
            foreach ($data as $k => $v){
                $html.='<tr><input type="hidden" name="mid[]" value="'.$v['itemsid'].'"><input type="hidden" name="mlid[]" value="'.$v['id'].'"><td>'.$v['name'].'</td><td>'.$v['unit'].'</td><td>'.$v['version'].'</td><td>'.$v['xysl'].'</td><td>'.$v['ckjg'].'</td><td>'.$v['sdate'].'</td><td>'.$v['edate'].'</td><td>'.$v['hire_day'].'</td><td>'.$v['xj'].'</td><td><a href="javascript:;" onclick="delMaterials(this,\''.$v['itemsid'].'\')">删除</a></td></tr>';
            }
