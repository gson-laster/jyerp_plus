...
use app\finance\model\Purchase as PurchaseModel;
...
//                      ['supplier_name', '供应商'],
	...
	public function get_Detail($source_id = ''){
			$data = MoneyModel::getOne($source_id);
//			halt($data);
//			echo $data->cid;
//			echo "<hr>";
//			echo $data->oid;
    		...
    		foreach ($data as $k => $v){ 
    			...
    			$html.='<tr><input type="hidden" name="mid[]" value="'.$v['wid'].'">
    				<input type="hidden" name="mlid[]" value="'.$v['id'].'">
    					...
    					<td>'.$v['unit'].'</td>
    					<td>'.$v['version'].'</td>
    					<td><input type="number" class="jg" oninput="input(this)" name="plan_num[]" value="'.$v['plan_num'].'"></td>
    					<td><input type="number" class="sl" oninput="input(this)" name="num[]" value="'.$v['num'].'"></td>
    					<td><input type="number" readonly="readonly" name="plan_money[]" class="zj" value="'.$v['plan_num']*$v['num'].'">
    					</td>
    					<td>'.$html2.'</td>
...
//                      ['static:3', 'supplier_username', '供应商签约人'],
	...
	public function delete($ids = null){		
		...
		$map['id'] = $ids;
//		halt($ids);
		if($model = PurchaseModel::where($map)->delete()){	
			//记录行为
...
EOF;
    ...
    public function tech($pid = '',$materials_list = '')
    {
        ...
        if($materials_list == '' || $materials_list == 'undefined') {
            ...
            $data = PurchaseModel::getMaterial($map);
