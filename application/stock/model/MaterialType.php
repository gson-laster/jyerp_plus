...
{
    // 设置当前模型对应的完整数据表名称
    protected $table = '__STOCK_MATERIAL_TYPE__';
...
public static function getMenuTree($id = 0, $default = '',$flag = 0)
	...
	public static function getOrganization(){
		$organization = self::field('id,title as text,pid')->select();
		$organizationall = self::getTree1(collection($organization)->toArray(),0);
		return json_encode($organizationall);
	}
		//递归目录父子数据
	public static function getTree1($data, $pid=0){
		$tree = array();
		foreach($data as $v){			
			if($v['pid'] == $pid){
				$v['nodes'] = self::getTree1($data, $v['id']);	
				if(empty($v['nodes'])) {
				 	$v['nodes'] = '';
				 }			
				unset($v['pid']);				
