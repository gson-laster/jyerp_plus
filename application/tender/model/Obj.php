 ...
 * 尾款*/
	...
	public static function balance_payment_one($map = [], $id){
		$list = Db::name('constructionsite_finish')  // 结束项目
		->alias('t')
		->field('t.item as id,b.money,tender_obj.contact,tender_obj.phone,b.name as contrack_name,tender_obj.name,t.file')
		->join('tender_obj','tender_obj.id=t.item','left')  // 项目
		...
		->group('t.item')
		->find();
			$list['all_money'] = $list['money'];
			$result = Db::name('finance_gather')-> where(['item_id' => $list['id']]) -> value('money');
			$list['pay'] = $result;
			$list['final_payment'] = $list['money']-$result;
			//dump($list);die;
		return $list;
	}
