...
class Hirecontract extends Model
{
    ...
    public static function getList($map = [], $order = [])
    {
        $data_list = self::view('tender_contract_hire')
            ...
            ->view("supplier_list", ['name'=>'objname'], 'supplier_list.id=tender_contract_hire.supplier', 'left')//供应商
            ->view('tender_obj',['name'=>'obname'],'tender_obj.id=tender_contract_hire.obj_id','left')//项目
            ->where($map)
            ->order($order)
            ->paginate();
        return $data_list;
