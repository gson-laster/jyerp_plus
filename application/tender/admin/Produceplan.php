 ...
 * @author HJP
 */
class Produceplan extends Admin
{
	//招标文件购买申请列表
	public function index()
	{
		...
		$map = $this->getMap();
		// 数据列表
		$data_list = ProduceplanModel::getList($map);
		//dump($data_list);die;               
		//获取昵称
		// 分页数据
...
public function task_list($id = null){
	...
	public function add(){
		...
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'Produceplan');
			// 验证失败 输出错误信息
			if(true !== $result) $this->error($result);
			//验证失败错误信息
			//查看人员，隔开
			
			if($model = ProduceplanModel::create($data)){
				//记入行为
