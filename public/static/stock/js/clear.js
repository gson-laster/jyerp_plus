...
if(pid){
	$.ajax({
			type: "GET",
			async: false,
			url: "/admin.php/stock/otherout/techs/pid/"+pid+"/materials_list/"+materials_list,
    ...
    $('#select').click(function(){
			...
			layer.open({
			  ...
			  area: ['70%', '70%'],
			  content: '/admin.php/tender/clear/choose_materials/materials/'+materials 
			});
	}); 
	//没有数据了选择按钮消失		
	...
	$('#pick').click(function(){
    		...
    		if($("#form_group_materials_name",parent.document).length>0){
    			...
    			html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td>'+$.trim($(this).find('td').eq(5).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td><input  type="number" class="xysl" oninput="inpu(this)" name="xysl[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';	    			  
	   			});
			}else{
				var html = '<div class="form-group col-md-12 col-xs-12" id="form_group_materials_name"><table class="table table-bordered"><tbody><tr><td>材料名称</td><td>计量单位</td><td>规格型号</td><td>需用数量</td><td>操作</td></tr>';
    		chk.each(function(){
    			ids += $(this).find('.ids').val()+',';   
    			html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><input type="hidden" name="mlid[]" value="0"><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td>'+$.trim($(this).find('td').eq(5).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td><input type="number" class="xysl" oninput="inpu(this)" name="xysl[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
   			});
