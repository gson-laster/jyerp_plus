$(function(){  
	//添加选中物品的按钮         	       
	//编辑获取
	    $('#form_group_materials_list').after('<div class="form-group col-md-12 col-xs-12" id="form_group_select"><button class="btn btn-xs btn-info" type="button" id="select">查看库存</button></div><div class="h_html"></div>');
	var pid = $("#id").val();
...
var obj_id = $('#obj_id').val();	
		$.ajax({
			type: "GET",
			async: false,
			url: "/admin.php/produce/mateplan/get_obj/obj_id/"+obj_id,
	...
	if(pid){
		$.ajax({
			type: "GET",
			async: false,
			url: "/admin.php/produce/mateplan/tech/ppid/"+pid+"/materials_list/"+materials_list,
			success: function(data){
