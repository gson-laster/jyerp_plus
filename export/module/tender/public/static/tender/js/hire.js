$(function(){
    ...
    $.ajax({
        type: "GET",
        async: false,
        url: "/admin.php/tender/hirecontract/tech/pid/"+pid+"/materials_list/"+materials_list,
    ...
    $('#select').click(function(){
        ...
        layer.open({
            ...
            area: ['70%', '70%'],
            content: '/admin.php/tender/hirecontract/choose_materials/materials/'+materials
        });
    });
    //没有数据了选择按钮消失
    ...
    $('#pick').click(function(){
        ...
        if($("#form_group_materials_name",parent.document).length>0){
            ...
            chk.each(function(){
                ...
                html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><td>'+$.trim($(this).find('td').eq(2).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td><input type="number" name="xysl[]"></td><td><input type="text" name="sdate[]"></td><td><input type="text" name="edate[]"></td><td><input type="text" name="hire_day[]"></td><td><input type="text" name="bz[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
            });
        }else{
            ...
            chk.each(function(){
                ids += $(this).find('.ids').val()+',';
                html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><input type="hidden" name="mlid[]" value="0"><td>'+$.trim($(this).find('td').eq(2).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td><input type="number" name="xysl[]"></td><td><input type="number" name="ckjg[]"></td><td><input type="date" name="sdate[]"></td><td><input type="date" name="edate[]"></td><td><input type="text" name="hire_day[]"></td><td><input type="number" name="xj[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
            });
