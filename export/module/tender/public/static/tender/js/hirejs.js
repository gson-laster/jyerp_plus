    ...
    $('#pick').click(function(){
        ...
        if($("#form_group_materials_name",parent.document).length>0){
            ...
            chk.each(function(){
                ...
                html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><td>'+$.trim($(this).find('td').eq(2).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td><input type="number" name="xysl[]"></td><td><td><input type="number" name="ckjg[]"></td><input type="text" name="sdate[]" class="js-datepicker" data-date-format="yyyy-mm-dd"></td><td><input type="text" name="edate[]" class="js-datepicker" data-date-format="yyyy-mm-dd"></td><td><input type="text" name="hire_day[]"></td><td><input type="text" name="xj[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
            });
        }else{
            ...
            chk.each(function(){
                ids += $(this).find('.ids').val()+',';
                html += '<tr><input type="hidden" name="mid[]" value="'+$.trim($(this).find('td').eq(1).text())+'"><input type="hidden" name="mlid[]" value="0"><td>'+$.trim($(this).find('td').eq(2).text())+'</td><td>'+$.trim($(this).find('td').eq(4).text())+'</td><td>'+$.trim($(this).find('td').eq(3).text())+'</td><td><input type="number" name="xysl[]"></td><td><input type="number" name="ckjg[]"></td><td><input type="text" name="sdate[]" class="js-datepicker" data-date-format="yyyy-mm-dd"></td><td><input type="text" name="edate[]" class="js-datepicker" data-date-format="yyyy-mm-dd"></td><td><input type="text" name="hire_day[]"></td><td><input type="number" name="xj[]"></td><td><a href="javascript:;" onclick="delMaterials(this,\''+$.trim($(this).find('td').eq(1).text())+'\')">删除</a></td></tr>';
            });
