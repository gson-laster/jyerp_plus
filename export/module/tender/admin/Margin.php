...
use app\tender\model\Margin as MarginModel;
...
class Margin extends Admin
{
	...
	public function index()
	{
		...
		$map = $this->getMap();
		// 数据列表
		$data_list = MarginModel::where($map)->paginate();
	...
	public function task_list($id = null){
		...
		$info = MarginModel::where('id',$id)->find();
...
$get_type = TypeModel::get_type();			
		...
		->addFormItems([
			...
			['select:6','name','项目名称','',$get_nameid,'','disabled'],
			['text:6','type','项目类型','','','','disabled'],
			['select:6','type2','项目类型','',$get_type,'','','hidden'],
			['select:6','type1','项目类型','',$get_typeid,'','','hidden'],			
			['select:6','unit1','建设单位','',$unit,'','','hidden'],
			['text:6','unit','收取单位','','','','disabled'],
			['text:6','bank','开户行','','','','disabled'],
			['text:6','account','账号','','','','disabled'],			
			['number:6','money','保证金金额','','','','','','disabled'],			
			['date:6','item_time','交款日期日期','','','','disabled'],
			['date:6','back_time','预计回款日','','','','disabled'],		
	...
	public function add(){
		...
		if($this->request->isPost()){
			$data = $this->request->post();
			// 验证
			$result = $this->validate($data, 'Margin');
			// 验证失败 输出错误信息
			...
			if($model = MarginModel::create($data)){
				//记入行为
...
$get_type = TypeModel::get_type();
		...
		->addFormItems([
			...
			['text:6','bank','开户行'],
			['text:6','account','账号'],			
			['number:6','money','保证金金额'],			
			['date:6','item_time','交款日期日期','',$date],
			['date:6','back_time','预计回款日'],									
			['file:6','path','文件路径'],
			['textarea','note','备注'],
