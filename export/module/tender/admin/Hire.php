...
//                action_log('tender_materials_add', 'tender_materials', $model['id'], UID);
    ...
    public function task_list($id = null){
        ...
        $info['hire_list'] = implode(HireModel::getMaterials($id),',');
        return ZBuilder::make('form')
            ->addGroup([
                '编辑发货'=>[
                    ['hidden','id'],
...
EOF;
    ...
    public function tech($pid = '',$materials_list = '')
    {
        if($materials_list == '' || $materials_list == 'undefined') {
            ...
            //dump($data);die;
            $html = '<div class="form-group col-md-12 col-xs-12" id="form_group_materials_name"><table class="table table-bordered"><tbody><tr><td>物品名称111</td><td>单位</td><td>规格</td><td>需用数量</td><td>单价</td><td>计划进场日期</td><td>计划退场日期</td><td>租赁天数</td><td>小计</td><td>操作</td></tr>';
            foreach ($data as $k => $v){
