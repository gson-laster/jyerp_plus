...
class Flowwork extends Admin
{
    ...
    public function add($fid=null){
        ...
        return ZBuilder::make('form')
            ...
            ->setFormItems($fields)
            ->addStatic('str_confirm', '审批步骤','',$this->_confirm($fid))
    ...
    public function ban($wid=null){
        ...
        return ZBuilder::make('form')
            ->setPageTitle('流程办理') // 设置页面标题
             ->addFormItems([
                ['static:6', 'title', '标题'],
                ['static:6', 'send_time', '申请时间','',date('Y-m-d H:i',$work->create_time)],
                ['static:6', 'username', '申请人','',get_nickname($work->user_id)],
                ['static:6', 'o_p', '部门职位','',$this->_oposition($work->user_id)],
                ['static:6','str_confirm', '审批步骤','',$this->_confirm($work->fid)],
                ['archive','add_file', '附件','',$work->add_file],
            ])
            ->setFormItems($fields)
    ...
    //拼接审批步骤人员 
    // type为空 return user_name的拼接,不为空未user_id的拼接 
